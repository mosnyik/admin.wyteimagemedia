          ...
          <TooltipProvider>
            <Tooltip>
              <TooltipTrigger asChild>
                <Checkbox
                  checked={
                    selectedContestants.length === contestants.length &&
            ...
            {contestants.length === 0 ? (
              ...
              contestants.map((contestant) => (
                ...
                >
                  ...
                  <td className="px-4 py-3">
                    <span
                      className={`inline-flex items-center px-2 py-1 rounded-full text-xs font-medium border ${getScreeningStatusColor(
                        contestant.screeningStatus
                      )}`}
                    >
                      {contestant.screeningStatus === "screened" && (
                        <Check className="w-3 h-3 mr-1" />
                      )}
                      {contestant.screeningStatus === "rejected" && (
                        <X className="w-3 h-3 mr-1" />
                      )}
                      {contestant.screeningStatus.charAt(0).toUpperCase() +
                        contestant.screeningStatus.slice(1)}
                    </span>
                  </td>
                  <td className="px-4 py-3 text-sm text-muted-foreground">
                    {contestant.appliedDate}
                  </td>
                  <td className="px-4 py-3">
                    <ContestantActions
                      ...
                      onToggleExpanded={() =>
                        setExpandedId(
                          expandedId === contestant.id ? null : contestant.id
                        )
                      }
                    />
                  </td>
